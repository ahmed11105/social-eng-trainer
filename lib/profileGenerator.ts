...
export function generateRealisticProfileWrapper(difficulty: Difficulty = 'easy', seed?: number): GeneratedProfile {
  ...
  const profile: Profile = {
    ...
    joinedDate: faker.date.past({ years: 5 }).toLocaleDateString('en-US', { month: 'long', year: 'numeric' }),
...
export function generateProfile(difficulty: Difficulty = 'easy', seed?: number): GeneratedProfile {
  ...
  const profile: Profile = {
    ...
    location: `${data.city}`,
    website: faker.internet.url(),
    joinedDate: new Date(data.joinedYear, faker.number.int({ min: 0, max: 11 })).toLocaleDateString('en-US', { month: 'long', year: 'numeric' }),
    following,
    followers,
    avatarUrl,
    coverUrl,
